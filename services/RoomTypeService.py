...
class RoomTypeService:
    ...
    def get_room_type_by_id(self, conn: sqlite3.Connection, room_type_id: int):
        return repo_get_by_id(conn, room_type_id)

    def list_room_types(self, conn: sqlite3.Connection):
        return repo_list(conn)
    ...
    def create_room_type(
        ...
        return repo_create(conn, name, capacity, base_price, description)
    ...
    def update_room_type(
        ...
        room_type = repo_get_by_id(conn, room_type_id)
        if not room_type:
            raise ValueError("Typ pokoje neexistuje")

        return repo_update(conn, room_type_id, name, capacity, base_price, description)
...
if __name__ == "__main__":
    ...
    with open_connection() as conn:
        ...
        rt = service.create_room_type(conn, "TestType1", 2, 1200.0, "desc", admin_role)
        print("Created:", rt)
        rt_id = rt["id"]

        print("\n=== TEST: create_room_type (receptionist FAIL) ===")
        try:
            service.create_room_type(conn, "TestType2", 3, 1500.0, None, rec_role)
        except PermissionError as e:
            print("Expected:", e)

        print("\n=== TEST: update_room_type (receptionist OK) ===")
        updated = service.update_room_type(conn, rt_id, rec_role, base_price=1300.0)
        print("Updated:", updated)

        print("\n=== TEST: update_room_type (customer FAIL) ===")
        try:
            service.update_room_type(conn, rt_id, cust_role, base_price=1500.0)
        except PermissionError as e:
            print("Expected:", e)

        print("\n=== TEST: delete_room_type (admin OK) ===")
        deleted = service.delete_room_type(conn, rt_id, admin_role)
        print("Deleted:", deleted)

        print("\n=== TEST: delete_room_type (receptionist FAIL) ===")
        try:
            service.delete_room_type(conn, rt_id, rec_role)
        except PermissionError as e:
            print("Expected:", e)
